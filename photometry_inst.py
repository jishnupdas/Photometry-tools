...
#%%
def Query(raImage, decImage,W):    #single use
    ...
    # 'II/349'#This is the catalog number of PS1 in Vizier
    # 'V/147'   SDSS
    # 'II/248'  2MASS
    ...
    try:
        ...
        #ascii.write(Q[0], 'cat_sources.csv', format='csv',overwrite=True)
        #print(Q[0])
        global ps1_imCoords
...
Qr,ps1_imCoords,ref_imCoords = Query(raImage, decImage,W)
try:
    ref_imCoords.write('reduced/sources/'+'reference_sources_cat.csv',
                       overwrite=True)
except:
...
def detected_sources_plot(imageName):
    ...
    ax = fig.gca()
    plt.imshow(data,
               vmin=np.median(data)-4*np.std(data),
...
def Curve_of_growth(imageName):
    ...
    plt.title(imageName)
    plt.imshow(psfModelData, vmin=0,
               vmax=np.median(data)+8*np.std(psfModelData))
